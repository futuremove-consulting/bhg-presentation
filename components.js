...
function renderGrowthChart(items) {
  ...
  const chartH = 286;
  const chartW = 800;
  const padL = 10, padR = 20, padB = 100, padT = 10;
  ...
  const bars = items.map((item, i) => {
    ...
    return `
      ...
      <text x="${x + barWidth / 2}" y="${chartH - padB + 24}" text-anchor="middle" fill="var(--text-secondary)" font-size="12" transform="rotate(-45, ${x + barWidth / 2}, ${chartH - padB + 24})">${label}</text>
    `;
  }).join('');
  ...
  const labels = items.map((item, i) => {
    ...
    return `<text x="${x}" y="${chartH - padB - h - 16}" text-anchor="middle" fill="var(--text)" font-size="12" font-weight="600">${item.growth}</text>`;
  }).join('');
